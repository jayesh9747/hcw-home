...
export class MediasoupServerController {
  constructor(private readonly mediasoupServerService: MediasoupServerService) {}
  ...
  @ApiResponse({ status: 409, description: 'Conflict - server URL already exists' })
  async create(
    @Body(new ZodValidationPipe(createMediasoupServerSchema))
    createMediasoupServerDto: CreateMediasoupServerDto,
    @Req() req: Request,
  ) {
    const server = await this.mediasoupServerService.create(createMediasoupServerDto);
    return ApiResponseDto.created(server, 'Mediasoup server created successfully', {
      requestId: req['id'],
      path: req.path,
    });
  }
  ...
  @Get()
  @ApiOperation({ summary: 'Get all mediasoup servers with pagination and filtering' })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({ name: 'search', required: false, type: String, description: 'Search term for URL or username' })
  @ApiQuery({ name: 'active', required: false, type: Boolean, description: 'Filter by active status' })
  @ApiQuery({
    name: 'sortBy',
    required: false,
    enum: ['url', 'username', 'maxNumberOfSessions', 'active', 'createdAt', 'updatedAt'],
  ...
  async findOne(@Param('id') id: string, @Req() req: Request) {
    ...
    return ApiResponseDto.success(server, 'Mediasoup server retrieved successfully', 200, {
      requestId: req['id'],
      path: req.path,
    });
  }
  ...
  async update(
    ...
    const server = await this.mediasoupServerService.update(id, updateMediasoupServerDto);
    return ApiResponseDto.success(server, 'Mediasoup server updated successfully', 200, {
      requestId: req['id'],
      path: req.path,
    });
  }
  ...
  async remove(@Param('id') id: string, @Req() req: Request) {
    ...
    return ApiResponseDto.success(server, 'Mediasoup server deleted successfully', 200, {
      requestId: req['id'],
      path: req.path,
    });
  }
