...
describe('ConsultationCleanupService', () => {
  ...
  beforeEach(async () => {
    ...
    mediasoupSessionService = {
      cleanupRouterForConsultation: jest.fn(),
      closeTransport: jest.fn(),
    };
    configService = {
      consultationRetentionHours: 24,
      consultationDeletionBufferHours: 1,
    };
    ...
    service = module.get<ConsultationCleanupService>(
      ConsultationCleanupService,
    );
  });
  ...
  it('should anonymize owner and patient participants before soft-deleting expired consultations', async () => {
    const consultations = [
      {
        ...
        deletionScheduledAt: new Date(Date.now() - 10000000),
        isDeleted: false,
      },
      {
        id: 2,
    ...
    db.participant.findMany.mockImplementation(({ where }) => {
      if (where.consultationId === 1)
        return Promise.resolve([{ userId: 101, role: UserRole.PATIENT }]);
      if (where.consultationId === 2)
        return Promise.resolve([
          { userId: 201, role: UserRole.PATIENT },
          { userId: 202, role: UserRole.PATIENT },
        ]);
      return Promise.resolve([]);
    });
    ...
    mediasoupSessionService.cleanupRouterForConsultation.mockResolvedValue(
      undefined,
    );
